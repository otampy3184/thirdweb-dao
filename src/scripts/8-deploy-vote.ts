...
(async () => {
  try {
    const voteContractAddress = await sdk.deployer.deployVote({
      ...
      // ERC-20 トークンのコントラクトアドレスを設定
      voting_token_address: "0x682DBa03EB560200984e67804d2884CeDb8F506c",

      // 以下の 2 つのパラメータはブロックチェーンのブロック数を指定（Ethereum の場合、ブロックタイムを 13-14 秒と仮定）
